...
jest.mock('./FavoriteButton', () => ({
  __esModule: true,
  default: ({ productId, variant }: { productId: string; variant?: string }) => (
    <button data-testid="favorite-btn" data-variant={variant}>{productId}</button>
  ),
}));
...
jest.mock('@/stores/useFavoriteStore', () => ({
  useFavoriteStore: (selector: (s: { favorites: Record<string, boolean> }) => boolean) =>
...
jest.mock('@/lib/utils', () => ({
  ...
  getLocalizedName: (obj: { name?: string } | null | undefined) => obj?.name || '',
}));
...
describe('ProductCard', () => {
  ...
  it('prefers explicit returnTo when provided', () => {
    render(<ProductCard product={mockProduct} returnTo="/en/search?q=sneaker" />);
    const link = screen.getByRole('link');
    expect(link).toHaveAttribute(
      'href',
  ...
  it('applies custom className', () => {
    ...
    // The cn utility will merge classNames
