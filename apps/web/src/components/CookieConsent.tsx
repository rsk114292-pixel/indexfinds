...
function writeConsentCookie(value: Exclude<ConsentStatus, 'pending'>): void {
  ...
  document.cookie =
    `${CONSENT_KEY}=${encodeURIComponent(value)}; path=/; max-age=${CONSENT_COOKIE_MAX_AGE}; SameSite=Lax`;
}
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 p-4 sm:p-6">
        ...
        >
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
            <button
              onClick={reject}
              className="rounded-md border border-border px-4 py-2 text-sm text-foreground transition-colors hover:bg-muted/50 cursor-pointer"
            >
              {t('reject')}
            </button>
            <button
    ...
    >
      <div className="container mx-auto flex items-center justify-between gap-4 px-4 py-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={reject}
            className="rounded-md px-3 py-1.5 text-sm text-muted hover:text-foreground transition-colors cursor-pointer"
          >
            {t('reject')}
          </button>
          <button
            onClick={accept}
            className="rounded-md bg-primary px-3 py-1.5 text-sm text-white hover:bg-primary-hover transition-colors cursor-pointer"
          >
            {t('accept')}
          </button>
        </div>
