...
function sortByPriority(categories: Category[]): Category[] {
  ...
  return [...categories].sort(
    (a, b) => (m.get(a.slug) ?? 999) - (m.get(b.slug) ?? 999),
  );
}
...
function CategoryCard({
  ...
  return (
    <div
      ...
      onKeyDown={(e) =>
        e.key === "Enter" && router.push(`/categories/${category.slug}`)
      }
      className={cn(
        ...
        {coverImage ? (
          <ImageWithFallback
            ...
            referrerPolicy={getImageReferrerPolicy(
              getProductCardThumbnail(coverImage),
            )}
          />
        ) : (
            ...
            >
              ...
              {children.length > (isLarge ? 6 : 4) && (
                <span className="px-2.5 py-1 text-xs text-white/50">
                  {t("moreCount", {
                    count: children.length - (isLarge ? 6 : 4),
                  })}
                </span>
              )}
            </div>
    ...
    >
      <div className="relative aspect-square overflow-hidden rounded-lg bg-gray-100">
        <ImageWithFallback
          ...
          loading="lazy"
          referrerPolicy={getImageReferrerPolicy(
            getProductCardThumbnail(product.mainImage),
          )}
        />
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        isLarge
          ? "col-span-2 lg:col-span-2 xl:col-span-3 h-52 sm:h-64 lg:h-72"
          : "col-span-1 lg:col-span-1 xl:col-span-2 h-40 sm:h-48 lg:h-56",
      )}
    />
  );
}
...
export default function CategoriesPageClient() {
  ...
  const { data: trendingData, isLoading: trendingLoading } = useSWR<
    ApiListResponse<TrendingProduct>
  >("/products?sortBy=popular&limit=12", fetcher);
